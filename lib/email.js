...
export function htmlPedido({ titulo, orden, datos, totales }) {
  const filas = Object.entries(datos)
    .map(
      ([k, v]) =>
        `<tr><td style="padding:6px 12px;font-weight:700;color:#000;">${k}</td><td style="padding:6px 12px;color:#5F5E5A;">${v}</td></tr>`
    )
    .join('');
  return `
  <div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;border:1px solid #e5e5e5;border-radius:12px;overflow:hidden;">
    <div style="background:linear-gradient(135deg, #00ae84 0%, #005261 100%);padding:20px 24px;">
      <p style="color:#ffffff;font-size:18px;margin:0;font-weight:700;">${titulo}</p>
    </div>
    <div style="padding:20px 24px;">
      <p style="font-size:14px;color:#333;"># de pedido: <strong style="color:#00AE84;">${orden}</strong></p>
