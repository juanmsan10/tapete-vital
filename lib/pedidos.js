...
const COLUMNAS = [
  ...
  'email', 'ciudad', 'direccion', 'notas', 'productos', 'guia', 'transportadora',
];
...
const alFormatoDeSiempre = (fila) => ({
  ...
  preguntado: fila.preguntado ? textoBogota(fila.preguntado) : '',
});
...
// Marca de que ya se le preguntó al cliente por esta entrega. Va directo a la
// tabla, sin espejo a la Sheet: es contabilidad interna, no un dato del pedido.
export async function marcarPreguntado(orden) {
  try {
    await sql`UPDATE pedidos SET preguntado = now() WHERE orden = ${orden}`;
  } catch (err) {
    console.error('[pedidos] No se pudo marcar preguntado:', err.message);
  }
}
...
export async function crearPedido(entrada) {
  ...
  await sql`INSERT INTO pedidos
    (orden, fecha, estado, cantidad, total, nombre, cedula, telefono, email, ciudad, direccion, notas, productos, origen)
