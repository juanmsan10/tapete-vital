...
// pares (número, estado). Cuando llegue el primer push real, el log dirá la
// forma verdadera y esto se podrá apretar.
export function guiasEntregadas(payload) {
  const encontradas = new Set();
  const visitar = (nodo) => {
    if (!nodo || typeof nodo !== 'object') return;
    if (Array.isArray(nodo)) return nodo.forEach(visitar);
    const estado = nodo.track_info?.latest_status?.status ?? nodo.latest_status?.status;
    if (nodo.number && String(estado).toLowerCase() === 'delivered') {
      encontradas.add(String(nodo.number).trim());
    }
