...
export async function registrarGuia(guia) {
  ...
  try {
    ...
    // Ya registrada: es el caso normal del barrido del cron, no un problema.
    // No descuenta cuota (verificado 3-sep-2026) y no vale la pena registrarlo.
    if (rechazo?.error?.code === -18019901) return { ok: true, yaEstaba: true };
    if (!res.ok || rechazo) {
