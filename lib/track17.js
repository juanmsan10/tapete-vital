...
// ============================================================
import { buscarPorGuia, marcarPreguntado } from '@/lib/pedidos';
import { preguntarPorEntrega, enviarCorreo, htmlPedido } from '@/lib/email';
    ...
    if (nodo.number && ultimo?.status) {
      ...
      const evento =
        [info?.latest_event, ...eventos]
          ...
          .find(Boolean) ?? '';
      // Cuándo lo recogió Inter. Es la fecha de despacho de verdad —la pone la
      // transportadora, no nosotros— y es contra la que se mide una demora.
      const recogida =
        eventos.find((e) => e?.stage === 'PickedUp') ?? eventos[eventos.length - 1];
      estados.set(String(nodo.number).trim(), {
        estado: String(ultimo.status),
        detalle: String(ultimo.sub_status || ''),
        evento: String(evento || '').trim(),
        despachado: recogida?.time_iso || recogida?.time_utc || null,
      });
    }
  ...
  for (const [guia, { estado, detalle, evento }] of estados) {
    ...
    for (const pedido of pedidos) {
      ...
      if (entregado) {
        ...
        await preguntarPorEntrega(pedido);
